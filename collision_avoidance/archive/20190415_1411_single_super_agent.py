...
class Collision_Avoidance_Env(gym.Env):
	...
	def __init__(self):
		...
		self.action_space = spaces.Box(low=-self.maxSpeed, high=self.maxSpeed, shape=(self.numAgents*2,), dtype=np.float32)

		self.observation_space = spaces.Box(low=-self.neighborDist, high=self.neighborDist, shape=(self.numAgents, 4 + self.laser_num*4), dtype=np.float32)
		self.seed()
	...
	def _get_obs(self):
		all_observations = []
		for agent_id in self.world["agents_id"]:
	...
	def done_test(self):
		for agent_id in self.world["agents_id"]:
			if self.agents_done[agent_id] == 0:
				pos = self.sim.getAgentPosition(self.world["agents_id"][agent_id])
				# target_pos = self.world["targets_pos"][agent_id]
	...
	def step(self, action):
		assert action.shape == (self.numAgents*2,)
		...
		for i in range(len(self.world["agents_id"])):
			...
			self.sim.setAgentPrefVelocity(agent_id, (float(action[2*i]),float(action[2*i+1])))
		...
		for i in range(len(self.world["agents_id"])):
			...
			# R_goal = np.dot(action[2*i:2*i+2], pref_vel)
			R_polite = np.dot(orca_vel, action[2*i:2*i+2])
		...
		assert observation.shape == (self.numAgents, 4 + self.laser_num*4)
	...
	def reset(self):
		for i in range(self.numAgents):
			...
			# angle = self.rng.uniform(0, 2 * pi)
			self.sim.setAgentPosition(agent_id, pos)
			self.world["targets_pos"][i] = (1.0,5.0)
